...
public class EntradaSalida {
    ...
    public String pedirDatoTexto(String mensaje) throws IOException {
        ...
        do {
            try {
                ...
                texto = pideCadena();
                if (texto.isEmpty()) {
                    throw new CadenaVaciaExcepcion("Debe introducir al menos un carácter");
                } else continuar = true;
            } catch (CadenaVaciaExcepcion cve) {
                mostrar("Error: "+cve.getMessage());
            }
        } while (!continuar);
        return texto;
    }
    ...
    public int pedirDatoNumero(String mensaje, int numeroMinimo) throws IOException {
        boolean continuar=false;
        int numero=0;
        do {
            try {
                ...
                mostrar("Error: Introduce un número mayor o igual que "+numeroMinimo);
            }
        } while (!continuar);
        return numero;
    }

    public int pedirDatoNumero(String mensaje, int numeroMinimo,int numeroMaximo) throws IOException {
        boolean continuar=false;
        int numero=0;
        do {
            try {
                ...
                mostrar("Error: Introduce un número entre "+numeroMinimo+" y "+numeroMaximo);
            }
        } while (!continuar);
        return numero;
    }
