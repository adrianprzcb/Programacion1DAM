...
public class Procesos {
    ...
    private void mostrarOrdenadas(Map<Integer,Integer> listaPuntuaciones) {
        ...
        for(int e=1; e<=listaPuntuaciones.size();e++){
         int numeroMayor = 0;
        int numeroActuacion= 0;

        for(int i=1; i<=listaPuntuaciones.size();i++){
            if(listaPuntuaciones.get(i)>numeroMayor){
                numeroMayor= listaPuntuaciones.get(i);
                numeroActuacion= i;
            }
        }

        System.out.println(listaConcursantes.get(numeroActuacion).getNombre());
        listaPuntuaciones.remove(numeroActuacion);
        }
